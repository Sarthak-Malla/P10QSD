...
Key fixes:
1. Ablation uses LR (best model), not RF
2. LR coefficients displayed properly
3. Rolling-window walk-forward (3yr window) instead of expanding
4. Multi-horizon comparison (5d, 10d, 20d)
5. Optimal threshold tuning for LR (not just 0.5)
6. No forced class_weight=balanced for RF (was hurting performance)
"""
...
def show_lr_coefficients(estimator, feature_cols):
    ...
    try:
        ...
        sc  = estimator.named_steps["sc"]
        coefs = clf.coef_[0]
        feat_names = feature_cols
        pairs = sorted(zip(feat_names, coefs), key=lambda x: -abs(x[1]))
...
def main(cfg: DictConfig):
    ...
    # Use last year of training as val for threshold tuning
    ...
    for name, info in tuned.items():
        ...
        results[name] = {"pred":pred,"acc":acc,"f1":f1,"p_val":p,"thresh":thresh}
    ...
    if len(all_proba) >= 2:
        ...
        results["ensemble"] = {"pred":ens_pred,"acc":ens_acc,"f1":ens_f1,"p_val":ens_p}
    ...
    # ── Ablation using LR (correct model) ─────────────────────────────
    logger.info("\n" + "="*60 + "\nABLATION STUDY (using LR)\n" + "="*60)
    ablation = {
        "price only":          [f for f in avail if f.startswith("price_")],
        "LM level only":       [f for f in avail if f.startswith("lm_") and not f.endswith("_delta") and "x_" not in f],
    ...
    for gname, feats in ablation.items():
        ...
        tr2 = dc[dc["filed_at"]<split_date]; te2 = dc[dc["filed_at"]>=split_date]
        if len(tr2)<30 or len(te2)<5: continue
        sc2 = StandardScaler()
        ...
        p2 = m2.predict(Xe)
        logger.info(f"  {gname:30s} acc={accuracy_score(te2[primary_target],p2):.4f}  "
                    f"f1={f1_score(te2[primary_target],p2,average='macro',zero_division=0):.4f}  n={len(feats)}")
